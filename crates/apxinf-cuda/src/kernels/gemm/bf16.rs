...
/// BF16 GEMM using one row of a larger activation matrix without materializing
/// logits for every prefill token.
pub fn gemm_bf16_last_row(
    ctx: &CudaContext,
    activation: &Tensor,
    weight: &Tensor,
) -> Result<Tensor> {
    if activation.dtype() != DType::BF16 || weight.dtype() != DType::BF16 {
        return Err(Error::Other("gemm_bf16_last_row expects BF16 operands".into()));
    }
    let ad = activation.shape().dims();
    let wd = weight.shape().dims();
    if ad.len() != 2 || wd.len() != 2 || ad[1] != wd[0] || ad[0] == 0 {
        return Err(Error::Other(format!(
            "gemm_bf16_last_row shape mismatch: {ad:?} @ {wd:?}"
        )));
    }
    let expected = Device::Cuda(ctx.device_id());
    if activation.device() != expected || weight.device() != expected {
        return Err(Error::DeviceMismatch {
            expected,
            got: if activation.device() != expected {
                activation.device()
            } else {
                weight.device()
            },
    ...
    let k = ad[1];
    let n = wd[1];
    let activation = CudaBuffer::from_tensor(activation)
        .map_err(Error::Cuda)?
        .view((ad[0] - 1) * k * DType::BF16.size_in_bytes(), k * DType::BF16.size_in_bytes())
        .map_err(Error::Cuda)?;
    let weight = CudaBuffer::from_tensor(weight).map_err(Error::Cuda)?;
    let output = output_buffer(ctx, n * DType::BF16.size_in_bytes())?;
    ctx.cublas()
        .gemm(DType::BF16, 1, n, k, 1.0, &activation, &weight, 0.0, &output)
        .map_err(Error::Cuda)?;
    Ok(output.into_tensor(Shape::new(vec![1, n]), DType::BF16))
}
