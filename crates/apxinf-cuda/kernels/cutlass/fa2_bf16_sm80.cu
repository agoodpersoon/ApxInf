...
void fill_params(FLASH_NAMESPACE::Flash_fwd_params& params, bool is_bf16,
                 ...
                 float softmax_scale, bool causal, bool kv_head_first,
                 int kv_capacity) {
  params = {};
  ...
  params.k_batch_stride = kv_head_first
      ? static_cast<int64_t>(kv_heads) * kv_capacity * head_dim
      : static_cast<int64_t>(key_tokens) * kv_row_stride;
  params.v_batch_stride = params.k_batch_stride;
  params.o_batch_stride = params.q_batch_stride;
  params.q_row_stride = q_row_stride;
  params.k_row_stride = kv_head_first ? head_dim : kv_row_stride;
  params.v_row_stride = kv_head_first ? head_dim : kv_row_stride;
  params.o_row_stride = q_row_stride;
  params.q_head_stride = head_dim;
  params.k_head_stride = kv_head_first
      ? static_cast<int64_t>(kv_capacity) * head_dim
      : head_dim;
  params.v_head_stride = kv_head_first
      ? static_cast<int64_t>(kv_capacity) * head_dim
      : head_dim;
...
int fa2(
    ...
    bool causal, bool kv_head_first, int kv_capacity, cudaStream_t stream) {
  ...
  fill_params(params, std::is_same<Element, cutlass::bfloat16_t>::value,
              ...
              causal, kv_head_first, kv_capacity);
  if (head_dim <= 96) {
    if (causal) {
...
int fa2_bf16(
  ...
  return fa2<cutlass::bfloat16_t>(
      ...
      false, false, key_tokens, stream);
}
int fa2_bf16_causal_headfirst_kv(
    const void* q, const void* k, const void* v, void* output,
    void* softmax_lse, int batch, int query_tokens, int key_tokens,
    int query_heads, int kv_heads, int head_dim, int kv_capacity,
    float softmax_scale, cudaStream_t stream) {
  return fa2<cutlass::bfloat16_t>(
      q, k, v, output, softmax_lse, batch, query_tokens, key_tokens,
      query_heads, kv_heads, head_dim, softmax_scale,
      true, true, kv_capacity, stream);
}
...
int fa2_f16(
  ...
  FLASH_NAMESPACE::Flash_fwd_params params;
  fill_params(params, false, q, k, v, output, softmax_lse, batch,
              query_tokens, key_tokens, query_heads, kv_heads, head_dim,
              softmax_scale, false, false, key_tokens);
  if (head_dim <= 96) {
    FLASH_NAMESPACE::run_mha_fwd_<cutlass::half_t, 96, false>(params, stream);
  } else {
