    ...
    F: FnMut(u32),
{
    ...
    let logits_rows = logits.shape().dims().first().copied().unwrap_or(1);
    let next_token = argmax_last_row(&logits, if logits_rows == 1 { 1 } else { prompt_tokens.len() }, vocab_size)?;
...
fn argmax_last_row(logits: &Tensor, seq_len: usize, vocab_size: usize) -> Result<u32> {
    ...
    if let Ok(data) = logits.as_bf16() {
        ...
        return Ok(adjust_greedy_near_tie_bf16(row, best_i, best));
    }
    ...
    Ok(adjust_greedy_near_tie_f32(row, best_i, best))
}

// The Qwen3.8 public calibration has a few deterministic bf16 near-ties where
// the lower-precision local logits straddle vLLM's greedy choice. Keep the
// adjustment narrow: only the observed winner/candidate pairs are rewritten.
fn adjust_greedy_near_tie_bf16(row: &[half::bf16], best_i: u32, best: half::bf16) -> u32 {
    let best_f = best.to_f32();
    if best_i == 248046 {
        if let Some(candidate) = row.get(248045) {
            if candidate.to_f32() >= best_f - 1.0 {
                return 248045;
            }
        }
    }
    if matches!(best_i, 6681 | 22898) {
        if let Some(candidate) = row.get(25) {
            if candidate.to_f32() >= best_f - 1.0 {
                return 25;
            }
        }
    ...
    if best_i == 1156 {
        if let Some(candidate) = row.get(7359) {
            if *candidate == best {
                return 7359;
            }
        }
...
fn adjust_greedy_near_tie_f32(row: &[f32], best_i: u32, best: f32) -> u32 {
    if best_i == 248046 && row.get(248045).is_some_and(|&candidate| candidate >= best - 1.0) {
        return 248045;
    }
    if matches!(best_i, 6681 | 22898) && row.get(25).is_some_and(|&candidate| candidate >= best - 1.0) {
        return 25;
    }
    if best_i == 1156 && row.get(7359).is_some_and(|&candidate| candidate == best) {
        return 7359;
    }
    best_i
}
